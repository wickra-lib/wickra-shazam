...
similarity over historical data, and places no orders. It holds no order-secret
material and opens no authenticated connections on its default path (the optional
`live` fingerprint feature only reads public data). The attack surface is
therefore narrow — principally the parsing of untrusted `FingerprintSpec` /
history data as it crosses the C ABI and WASM boundary. See [THREAT_MODEL.md](THREAT_MODEL.md) for the asset
...
buffer protocol, denial-of-service through a hostile `FingerprintSpec` or dataset (for
...
affecting `wickra-shazam` is documented there with a reason, so downstream
