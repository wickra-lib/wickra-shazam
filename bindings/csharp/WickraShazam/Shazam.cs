...
/// <summary>
/// A shazam instance driven by JSON commands, over the Wickra C ABI. Build one
...
/// </summary>
public sealed class Shazam : IDisposable
{
    private readonly ShazamHandle _handle;

    /// <summary>Build a shazam from a spec JSON string.</summary>
    /// <exception cref="ArgumentException">The spec is null/invalid.</exception>
    public Shazam(string specJson)
    {
        IntPtr ptr = Native.wickra_shazam_new(Utf8(specJson));
        if (ptr == IntPtr.Zero)
        {
            throw new ArgumentException("wickra-shazam: invalid spec", nameof(specJson));
        }
        _handle = new ShazamHandle(ptr);
    }
    ...
    public string Command(string cmdJson)
    {
        ...
        int n = Native.wickra_shazam_command(h, cmd, null, 0);
        if (n < 0)
        {
            throw new InvalidOperationException($"wickra-shazam: command failed (code {n})");
        }
        var buf = new byte[n + 1];
        Native.wickra_shazam_command(h, cmd, buf, (nuint)buf.Length);
        return Encoding.UTF8.GetString(buf, 0, n);
    }

    /// <summary>The library version.</summary>
    public static string Version() =>
        Marshal.PtrToStringUTF8(Native.wickra_shazam_version()) ?? string.Empty;

    /// <summary>Free the native shazam handle.</summary>
...
/// <summary>A safe handle owning a native shazam pointer.</summary>
internal sealed class ShazamHandle : SafeHandle
{
    public ShazamHandle(IntPtr handle)
    ...
    {
        Native.wickra_shazam_free(handle);
        return true;
    }
}
